...
class Detour_ext {
	...
	function sessions_start() {
        ...
        // We use two different methods of determining our ACTUAL uri,
        // because they may give different results depending on the server configuration.
		$segs   = $this->EE->db->escape_str(trim($this->EE->uri->uri_string, '/'));
		$url    = $this->EE->db->escape_str(trim($_SERVER['REQUEST_URI'], '/'));
        
        // Build a chunk of SQL, based on whether our options end up the same or different
        $searchstring   = ($segs == $url) 
                            ? "original_url = '{$segs}'" 
